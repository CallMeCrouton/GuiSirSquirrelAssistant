...
def main(runs=None, difficulty=None, x_offset_shared=None, y_offset_shared=None):
    ...
    try:
        ...
        logger.info(f"Connection monitoring started for Threads automation")
        
        for i in range(runs):
            try:
                logger.info(f"Starting Threads run {i+1}/{runs}")
                ...
                while True:
                    if connection_manager.connection_event.is_set():
                        ...
                        logger.info(f"Completed Threads run {i+1}/{runs}")
                        break
                    else:
                        # Connection lost, wait for it to be restored
                        logger.warning(f"Connection lost during Threads run {i+1}, waiting for reconnection...")
                        connection_manager.connection_event.wait()
                        logger.info(f"Connection restored, resuming Threads run {i+1}")
                    
                    # Check for server errors (copied from compiled_runner)
                    try:
                        from common import element_exist
                        if element_exist("pictures/general/server_error.png"):
                            logger.warning(f"Server error detected during Threads run {i+1}")
