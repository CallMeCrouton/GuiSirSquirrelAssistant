...
def main(runs, stage, x_offset_shared=None, y_offset_shared=None):
    ...
    try:
        ...
        logger.info(f"Connection monitoring started for Exp automation")
            ...
            try:
                ...
                while True:
                    if connection_manager.connection_event.is_set():
                        ...
                        logger.warning(f"Connection lost during Exp run {i+1}, waiting for reconnection...")
                        connection_manager.connection_event.wait()
                        logger.info(f"Connection restored, resuming Exp run {i+1}")
                    
                    # Check for server errors (copied from compiled_runner)
                    try:
                        from common import element_exist
                        if element_exist("pictures/general/server_error.png"):
                            logger.warning(f"Server error detected during Exp run {i+1}")
